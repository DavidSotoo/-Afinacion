  ...
  parser.getText({ first: 19, last: 33 }).then(function(result) {
    ...
    result.pages.forEach(p => {
      ...
      lines.forEach(line => {
        ...
        if (yearIndex === -1) return; // not a vehicle row
        ...
        if (litToken) {
          ...
          // If no L-suffix, look for numbers like 5.7 or 4.3 or 3.0
        ...
        restTokens.forEach(t => {
          // Clean token from trailing parts like (STD or (1 or (2 or /CA (wait, /CA is part of cabin filter)
          ...
          if (/^(FO|OF)-\w+/i.test(cleanToken)) {
            ...
            // Cabin (F- ends with /CA or is cabin)

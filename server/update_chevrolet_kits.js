...
function buildFilterObject(sku, filterTypeLabel) {
  if (!sku) return null;
  // If SKU contains 'SELLADO', handle accordingly
  ...
  try {
    ...
    for (const dbV of dbVehicles) {
      ...
      if (matches.length > 0) {
        ...
        // Initialize kit_afinacion if null
        ...
        // Apply filters_unifil fields (for backward compatibility)
    ...
    console.log(`\nSuccessfully updated ${updatedCount} Chevrolet vehicles.`);

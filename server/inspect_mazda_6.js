...
mongoose.connect("mongodb+srv://taskflowUser:REDACTED_PASSWORD@cluster0.adtp9yw.mongodb.net/?appName=Cluster0")
  .then(async () => {
    const v = await Vehiculo.findOne({ marca: /mazda/i, modelo: /6/i });
    if (v) {
      ...
      console.log("BUJIAS DATA:", JSON.stringify(v.bujia_stock || v.bujia_iridium_ix, null, 2));
    } else {
      console.log("NO MAZDA 6 FOUND");
    }
    process.exit(0);
  })
  .catch(err => {

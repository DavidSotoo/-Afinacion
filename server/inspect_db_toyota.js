...
async function main() {
  try {
    ...
    const vehicles = await Vehiculo.find({ marca: /toyota/i });
    console.log(`Total Toyota vehicles in DB: ${vehicles.length}`);
    ...
    console.log('\n=== UNIQUE TOYOTA MODELS IN DB ===');
    console.log(Array.from(uniqueModels).sort());

    console.log('\n=== ENGINES FOR MAIN TOYOTA MODELS ===');
    const mainModels = ['AVANZA', 'CAMRY', 'COROLLA', 'HILUX', 'HIACE', 'PRIUS', 'RAV4', 'SIENNA', 'TACOMA', 'TUNDRA', 'YARIS'];

...
function matchModel(dbModel, pdfModel) {
  ...
  // Specific MR2 SPYDER -> MR2
  if (dbClean === 'MR2 SPYDER' && pdfClean === 'MR2') return true;
  if (dbClean === 'MR2' && pdfClean === 'MR2 SPYDER') return true;
...
async function main() {
  try {
    ...
    const dbVehicles = await Vehiculo.find({ marca: /toyota/i });
    console.log(`Total Toyota vehicles in DB: ${dbVehicles.length}`);

    const dataBuffer = fs.readFileSync(pdfPath);
    const parser = new PDFParse({ data: dataBuffer });
    const result = await parser.getText({ first: 70, last: 73 });

...
async function main() {
  try {
    ...
    const vehicles = await Vehiculo.find({ marca: /ford/i });
    console.log(`Total Ford vehicles in DB: ${vehicles.length}`);
    ...
    console.log('\n=== UNIQUE FORD MODELS IN DB ===');
    console.log(Array.from(uniqueModels).sort());

    console.log('\n=== ENGINES FOR MAIN FORD MODELS ===');
    const mainModels = ['F150', 'F-150', 'F250', 'F-250', 'F350', 'F-350', 'FESTIVA', 'FIESTA', 'FIGO', 'FOCUS', 'ECOSPORT', 'ESCAPE', 'EXPEDITION', 'EXPLORER', 'RANGER', 'MUSTANG', 'IKON', 'KA'];

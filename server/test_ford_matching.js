...
function matchModel(dbModel, pdfModel) {
  ...
  // 3. F-150 / F-250 / F-350 / F-450 / F-550 hyphens (e.g. F150 -> F-150)
...
// Function to extract all float displacement numbers from a motor string
  ...
  try {
    ...
    result.pages.forEach(p => {
      ...
      lines.forEach(line => {
        ...
        // Find motor displacement string
    ...
    for (const dbV of dbVehicles) {
      ...
      const matches = pdfRecords.filter(r => {
        ...
        // Litros matching: support array of numbers
        let litrosMatches = false;
        if (r.litrosList.length > 0) {
          litrosMatches = r.litrosList.some(v => Math.abs(v - dbLitros) < 0.05);
        } else {
          // If no displacement found in PDF record, match anyway as fallback if year overlaps

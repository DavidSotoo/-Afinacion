...
function matchModel(dbModel, pdfModel) {
  ...
  // Prefix / Suffix matching (covers: CIVIC TYPE R -> CIVIC, CIVIC HYBRID -> CIVIC, ACCORD CROSSTOUR -> ACCORD)
...
async function main() {
  try {
    ...
    const dbVehicles = await Vehiculo.find({ marca: /honda/i });
    console.log(`Total Honda vehicles in DB: ${dbVehicles.length}`);

    const dataBuffer = fs.readFileSync(pdfPath);
    const parser = new PDFParse({ data: dataBuffer });
    const result = await parser.getText({ first: 48, last: 49 });
